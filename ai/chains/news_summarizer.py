...
from ai.prompts.news_prompts import (
    NewsSummaryOutput,
    ImpactedStock,
...
def _build_news_summary_chain(provider: str = "openai", model_name: Optional[str] = None):
    """
    ニュース要約チェーンを構築する。
...
def summarize_news_article(
    ...
    """
    logger.info(f"ニュース要約開始: provider={provider}, title='{title[:50]}...'")
...
def summarize_news_batch(
    ...
    logger.info(f"ニュースバッチ要約開始: {len(articles)}件, provider={provider}")
    ...
    for idx, article in enumerate(articles):
        ...
        try:
            ...
            results.append({
                ...
                "summarized_content": "要約の生成に失敗しました。",
                "sentiment": "neutral",
