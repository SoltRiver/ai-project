...
def integrate_nikkei_data():
    ...
    try:
        ...
        try:
            ...
            df = pd.read_csv(csv_path, encoding='utf-8', skiprows=0) # Removing skiprows=1 because my sample HAS header at line 1
    ...
    for code in sample_list:
        try:
            ...
            address = data['所在地']
            capital_str = str(data['資本金']).replace(',', '') if not pd.isna(data['資本金']) else None
            capital = int(capital_str) if capital_str and capital_str.isdigit() else None
            ...
            if internal_sector_name:
                sector = db.query(company_info.Sector).filter_by(name=internal_sector_name).first()
                if not sector:
            ...
            if not stock_master:
                stock_master = master.StockMaster(code=code, name=corporate_name) # Basic info
                db.add(stock_master)
                db.commit()
