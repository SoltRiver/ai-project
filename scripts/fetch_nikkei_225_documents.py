...
def fetch_nikkei_documents():
    ...
    for company in companies:
        ...
        if found_doc:
            ...
            print(f"  FOUND Document: {doc_id} / {submitter_name} / PeriodEnd: {period_end_str}")
            ...
            if existing and existing.status == "OK":
                ...
                if not existing.period_end and period_end_str:
                     existing.period_end = datetime.strptime(period_end_str, "%Y-%m-%d").date()
            ...
            print(f"  No Annual Report (120) found in scan window ({start_date.strftime('%Y-%m-%d')} - {SCAN_DAYS} days back).")
