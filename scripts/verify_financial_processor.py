...
def verify():
    ...
    doc = EdinetDocument(
        doc_id=doc_id, target_date=datetime.date(2024, 6, 25), filer_name="Test Corp"
    )
    db.add(doc)
    db.commit()
    ...
    if sales:
        print(
            f"[NetSales] Conf: {sales.confidence}, Val: {sales.value_numeric}, Dur: {sales.duration_days}, Label: '{sales.metric_label}'"
        )
    ...
    if op:
        print(
            f"[OpProfit] Conf: {op.confidence}, Val: {op.value_numeric}, Dur: {op.duration_days}"
        )
