...
def process_pdf_batch(limit: int = 10):
    ...
    try:
        ...
        qt = (
            db.query(EdinetFile.doc_id)
            .outerjoin(
                EdinetPdfExtractStatus,
                EdinetFile.doc_id == EdinetPdfExtractStatus.doc_id,
            )
            .filter(EdinetFile.file_type == "PDF_TYPE2")
            .filter(EdinetFile.status == "OK")
            .filter(
                (EdinetPdfExtractStatus.status == None)
                | (EdinetPdfExtractStatus.status != "OK")
            )
            .limit(limit)
        )
        ...
        for i, doc_id in enumerate(doc_ids):
            ...
            if status == "OK":
                print(
                    f"  Success: {result.get('page_count')} pages, {result.get('total_chars')} chars"
                )
                success_count += 1
            elif status == "SKIP":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of docs to process"
    )
