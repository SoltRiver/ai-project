...
def process_timeseries_batch(sec_code=None, limit=None):
    ...
    try:
        if sec_code:
            ...
            stmt = text(
                "SELECT DISTINCT sec_code FROM edinet_documents WHERE sec_code IS NOT NULL"
            )
            if limit:
                stmt = text(
                    f"SELECT DISTINCT sec_code FROM edinet_documents WHERE sec_code IS NOT NULL LIMIT {limit}"
                )
