...
- **外部APIとの連携におけるパーセンテージ表記の補正**: yfinanceなどの外部APIから配当利回り等の比率データを取得する際、データソースやAPIのバージョン・対象銘柄によって「小数値（例: 0.0361）」で返る場合と「パーセント生値（例: 3.61）」で返る場合が混在し得る。一律で `100` 倍して `%` を付与すると「361%」等の巨大な異常表示になるため、`0.5`（50%）などを閾値とした条件分岐を設け、小数の場合のみ 100倍するように実装し、表示の不整合を防ぐ。
