...
def test_ai_prediction():
    ...
    if result["probability"] != "-":
        assert "validation" in result, "Result with probability should contain validation info"
        assert 0 <= result["probability"] <= 1, f"Probability {result['probability']} should be between 0 and 1"
    else:
