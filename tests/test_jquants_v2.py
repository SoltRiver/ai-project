...
def test_get_listed_issues_success(client):
    ...
    mock_response.json.return_value = {
        "data": [
            {"Code": "72030", "CoName": "Toyota", "MktNm": "Prime"}
        ]
    }
...
def test_get_daily_quotes_mapping(client):
    ...
    mock_response.json.return_value = {
        "data": [
            {"Date": "20240101", "Code": "72030", "O": 100, "H": 110, "L": 90, "C": 105, "Vo": 1000}
        ]
    }
    ...
    with patch('requests.get', return_value=mock_response):
        ...
        assert q["High"] == 110
        assert "Volume" in q
        assert "O" not in q # Should be removed/mapped? Logic said "new_item[new_k] = new_item.pop(old_k)"
