...
- **実装完了後（コミット前）、速やかに `codex` コマンド（対話モード）を起動する**
- **`/review` コマンドを入力し、選択肢が表示されたら `2` (Review uncommitted changes) を選択してレビューを実施する**
- **Codexによるレビューは、10分以上かかる場合を除き、必ず完了を待ってから次のアクションを行う**
- **許可を求められた場合は、すべて `Yes` と回答する**
- **レビューの結果、エラーや修正点が検出された場合は、コミット・プッシュを行わず、`codex` に修正を依頼する**
- **資材の修正が完了した後も、必ずCodexによるレビュー(`/review` コマンド)を再度実施する（この手順は省略不可）**
- **再度レビューを行い、問題がなければコミット & プッシュを行う**
