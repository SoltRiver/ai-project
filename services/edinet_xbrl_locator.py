...
class EdinetXbrlLocator:
    async def locate_xbrl_files(self, directory: Path) -> Dict[str, Any]:
        ...
        public_docs = [f for f in xbrl_files if "PublicDoc" in f or "PublicDoc" in os.path.dirname(f)]
        if public_docs:
        ...
        if not primary_xbrl and inline_xbrl_files:
             ...
             public_inlines = [f for f in inline_xbrl_files if "PublicDoc" in f or "PublicDoc" in os.path.dirname(f)]
             if public_inlines:
