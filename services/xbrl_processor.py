...
class XbrlProcessor:
    ...
    def process_document(self, doc_id: str) -> Dict[str, Any]:
        ...
        ef = (
            ...
            .filter_by(doc_id=doc_id, file_type="ZIP_TYPE1", status="OK")
            .first()
        )
        if not ef:
            return self._build_result(
                doc_id, status="SKIP", msg="No ZIP_TYPE1 found or not OK"
            )
    ...
    def _unzip_safely(self, zip_path: Path, dest_dir: Path):
        ...
        with zipfile.ZipFile(zip_path, "r") as z:
            for member in z.infolist():
                ...
                is_safe = (
                    not member.filename.startswith("/") and ".." not in member.filename
                )
                if not is_safe:
                    print(
                        f"Warning: Suspicious file path in zip {member.filename}. Skipping."
                    )
                    continue
    ...
    def _detect_candidates(self, scan_dir: Path) -> Dict[str, Any]:
        ...
        for file_path in scan_dir.rglob("*"):
            ...
            if ext in [".xbrl", ".xml"]:
                ...
                if size > 100:  # Min size filter
                    xbrl_cands.append(
                        {"path": str(file_path), "size": size, "rel_path": rel_path}
                    )
        ...
        return {
            "xbrl_candidates": sorted(
                xbrl_cands, key=lambda x: x["size"], reverse=True
            ),
            "ixbrl_candidates": sorted(
                ixbrl_cands, key=lambda x: x["size"], reverse=True
            ),
        }
        ...
        try:
            ...
            for ctx in soup.find_all(["xbrli:context", "context"]):
                ...
                if ent:
                    ...
                    if ident:
                        c_data["entity"] = ident.text.strip()
    ...
    def _parse_ixbrl_html(self, file_path: str) -> List[Dict]:
        ...
        try:
            ...
            tags = soup.find_all(
                ["ix:nonnumeric", "ix:nonfraction", "ix:nonNumeric", "ix:nonFraction"]
            )
    ...
    def _bulk_insert_facts(self, doc_id: str, facts: List[Dict]) -> int:
        ...
        for f in facts:
            ...
            obj = EdinetXbrlFact(
                ...
                concept=f["concept"],
                value_text=(
                    f["value_text"][:20000] if f["value_text"] else None
                ),  # Truncate large text
    ...
    def _record_failure(
        self, doc_id, zip_path, ext_dir, msg, log_path, candidates=None, primary=None
    ):
        ...
        res = {
            ...
            "extracted_dir": str(
                Path(ext_dir).relative_to(BASE_STORAGE_DIR)
                if BASE_STORAGE_DIR in Path(ext_dir).parents
                else ext_dir
            ),
            "detected": candidates,
            "selected_primary": primary,
