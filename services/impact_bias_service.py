...
class ImpactBiasService:
    ...
    def get_impact_bias(self, sec_code: str) -> Dict[str, Any]:
        ...
        # 1. Fetch News
        news_query = self.db.query(StockNews, StockNewsAnalysis).join(StockNewsAnalysis)\
            .filter(StockNews.sec_code == sec_code)\
            .filter(StockNews.published_at >= cutoff_date)
            
        # 2. Fetch Events
        event_query = self.db.query(StockEvent, StockEventAnalysis).join(StockEventAnalysis)\
