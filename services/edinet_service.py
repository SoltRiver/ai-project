...
class EdinetClient:
    ...
    def _load_code_map(self) -> pd.DataFrame:
        ...
        return (
            pd.DataFrame()
        )  # Fallback: return empty and rely on other methods or manual update
    ...
    def search_annual_report(
        self, edinet_code: str, reference_date: Optional[datetime] = None
    ) -> Optional[str]:
        """
        ...
        found = self._scan_period(
            edinet_code, reference_date, days=5
        )  # Reduced from 30
        if found:
            return found
    ...
    def _scan_period(
        self, edinet_code: str, date: datetime, days: int = 1
    ) -> Optional[str]:
        """
                ...
                if res.status_code == 200:
                    ...
                    if docs:
                        ...
                        if "statusCode" in res.text and "401" in res.text:
                            print(
                                "WARNING: EDINET API Access Denied (Missing/Invalid Key). Switching to fallback mode."
                            )
    ...
    def get_documents_by_date(
        self, date_obj: datetime, type_code: int = 2
    ) -> List[Dict[str, Any]]:
        """
    ...
    def download_and_parse(self, doc_id: str) -> Dict[str, Any]:
        ...
        for root, dirs, files in os.walk(xbrl_dir):
            for file in files:
                ...
                if (
                    file.endswith(".xbrl") or file.endswith(".xml")
                ) and "PublicDoc" in root:
    ...
    def _normalize_financials(self, data: Any) -> Dict[str, Any]:
        ...
        targets = {
            "sales": [
                ...
                "NetSalesSummaryOfBusinessResults",  # J-GAAP/USGAAP
                "SalesRevenuesIFRS",
    ...
    def fetch_edinet_code_list(self) -> Optional[str]:
        ...
        try:
            ...
            if (
                content.strip().startswith(b"<!DOCTYPE")
                or b"<html" in content[:100].lower()
            ):
                print(
                    "Downloaded content appears to be HTML (likely error page). Skipping."
                )
                return None
