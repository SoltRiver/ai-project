...
class FinancialProcessor:
    def __init__(self, db: Session = None, config_path: str = "config/edinet_metrics.yml"):
    ...
    def process_document(self, doc_id: str):
        ...
        for metric in metrics_def:
            ...
            for priority, concept in enumerate(candidates):
                ...
                for f in facts:
                    ...
                    if priority == 0 and is_duration_ok and has_val:
                        ...
                        curr_conf = "MID"
                        reason_parts = []
                        if priority > 0: reason_parts.append(f"Fallback {concept}")
                        if not is_duration_ok: reason_parts.append(f"Duration {duration} mismatch")
            ...
            if not best_fact:
                ...
                p_type = "duration" if dur is not None else ("instant" if best_fact.instant_date else "unknown")
                ...
                results.append({
                    ...
                    "period_end": best_fact.period_end or best_fact.instant_date, # Use instant as end?
        ...
        for l in logs:
            if l["result"] == "NOT_FOUND":
                print(f"[WARN] Missing {l['metric_key']} in {doc_id}. Conf: {l['confidence']}")
            else:
                # Optional info log
                pass
    ...
    def _upsert_highlights(self, doc_id: str, data: List[Dict]):
        ...
        for item in data:
            # Check
            existing = self.db.query(EdinetFinancialHighlight).filter_by(
                doc_id=doc_id, metric_key=item["metric_key"]
            ).first()
            ...
            if existing:
                ...
                print(f"[DEBUG] Inserting: key={item.get('metric_key')}, label={item.get('metric_label')}")
                
                if "metric_label" not in item or item["metric_label"] is None:
                     print(f"[ERROR] metric_label IS NONE OR MISSING: {item}")
                
                new_rec = EdinetFinancialHighlight(
                    doc_id=doc_id,
                    **item
                )
                self.db.add(new_rec)
