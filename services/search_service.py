...
class SearchService:
    ...
    def search(self, query: str, 
        ...
        if q_len == 2:
            ...
            if not sec_code and not (date_from and date_to):
                return {"status": "ERROR", "message": "For 2-char queries, specify Date Range or Sec Code"}
        ...
        stmt = self.db.query(
            ...
            EdinetDocument, EdinetPdfText.doc_id == EdinetDocument.doc_id
        )
        ...
        try:
            ...
            for r in rows:
                ...
                results.append({
                    "doc_id": r.doc_id,
                    "page_no": r.page_no,
                    "submit_date": r.submit_datetime.isoformat() if r.submit_datetime else None,
    ...
    def _generate_snippet(self, text_body: str, query: str) -> str:
        ...
        """
        if not text_body or not query: return ""
