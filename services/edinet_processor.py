...
class EdinetProcessor:
    ...
    def _upsert_documents_metadata(self, target_date: date, docs_list: List[dict]):
        ...
        for data in docs_list:
            try:
                ...
                # Parse dates
                def p_date(s): return datetime.strptime(s, "%Y-%m-%d").date() if s else None
                def p_datetime(s): return datetime.strptime(s, "%Y-%m-%d %H:%M").replace(hour=int(s[-5:-3]), minute=int(s[-2:])) if s and len(s)>10 else None 
                ...
                doc_record = EdinetDocument(
                    ...
                    submit_datetime=datetime.strptime(data.get("submitDateTime"), "%Y-%m-%d %H:%M") if data.get("submitDateTime") else None,
                ...
                existing = self.db.query(EdinetDocument).filter_by(doc_id=doc_id).first()
                if existing:
                    # Update fields
    ...
    def _filter_documents_mvp(self, target_date: date) -> List[EdinetDocument]:
        ...
        all_docs = self.db.query(EdinetDocument).filter_by(target_date=target_date).all()
        ...
        for doc, prio in candidates:
            key = doc.edinet_code # Better than sec_code which might be null for some filers?
            if not key: key = doc.doc_id # Fallback
        ...
        for d in final_list:
            print(f"- {d.doc_id} / {d.edinet_code} / {d.doc_description} ({d.submit_datetime})")
    ...
    def _handle_download(self, doc: EdinetDocument, file_type_str: str, api_type_code: int):
        doc_id = doc.doc_id
        
        # 1. Idempotency Check
        existing = self.db.query(EdinetFile).filter_by(doc_id=doc_id, file_type=file_type_str).first()
        if existing and existing.status == "OK":
            ...
            if abs_path.exists():
                ...
                logger.warning(f"Record OK but file missing for {doc_id}. Re-downloading.")
        ...
        try:
            ...
            if content:
                ...
                date_ref = doc.submit_datetime if doc.submit_datetime else datetime.combine(doc.target_date, datetime.min.time())
    ...
    def _upsert_ng_record(self, doc_id, file_type, msg):
        ...
        # We handle NG here.
        existing = self.db.query(EdinetFile).filter_by(doc_id=doc_id, file_type=file_type).first()
        if existing:
            existing.status = "NG"
