...
class DerivedMetricProcessor:
    ...
    def process_document(self, doc_id: str):
        ...
        """
        # 1. Fetch Highlights
        highlights = self.db.query(EdinetFinancialHighlight).filter_by(doc_id=doc_id).all()
        if not highlights:
        ...
        self._calc_margin(doc_id, metrics_map, "operating_margin", "operating_profit", "net_sales", "営業利益率")
        self._calc_margin(doc_id, metrics_map, "net_margin", "profit_attributable_to_owners", "net_sales", "純利益率")

        # B. Structure (Instant / Instant)
        self._calc_structure(doc_id, metrics_map, "equity_ratio", "equity", "total_assets", "自己資本比率")

        # C. Return (Duration / Instant - Mixed Approx)
        self._calc_return(doc_id, metrics_map, "roe_end", "profit_attributable_to_owners", "equity", "ROE（期末近似）")
        self._calc_return(doc_id, metrics_map, "roa_end", "profit_attributable_to_owners", "total_assets", "ROA（期末近似）")
    ...
    def _calc_margin(self, doc_id, metrics_map, derived_key, num_key, den_key, label):
        ...
        self._calculate_generic(
            doc_id, metrics_map, derived_key, label, num_key, den_key, 
            rule_type="margin"
        )

    def _calc_structure(self, doc_id, metrics_map, derived_key, num_key, den_key, label):
    ...
    def _calculate_generic(self, doc_id, metrics_map, derived_key, label, num_key, den_key, rule_type):
    ...
    def _validate(self, num, den, rule_type) -> Dict:
        ...
        if u1 != u2:
            return {"valid": False, "confidence": "LOW", "reason": f"Unit mismatch: {u1} vs {u2}"}
        
        if "JPY" not in u1 and "円" not in u1:
             # MVP restriction to JPY
            return {"valid": False, "confidence": "LOW", "reason": f"Non-JPY unit: {u1}"}
        ...
        if rule_type == "margin":
            ...
            if p1 != "duration" or p2 != "duration":
                return {"valid": False, "confidence": "LOW", "reason": f"Period type mismatch for margin: {p1}/{p2}"}
            # Days Check
            d1 = num.duration_days
            d2 = den.duration_days
            if d1 != d2:
                 return {"valid": False, "confidence": "LOW", "reason": f"Duration days mismatch: {d1} vs {d2}"}

        elif rule_type == "structure":
             # Instant / Instant
             if p1 != "instant" or p2 != "instant":
                 return {"valid": False, "confidence": "LOW", "reason": f"Period type mismatch for structure: {p1}/{p2}"}

        elif rule_type == "return":
             # Duration / Instant
             if p1 != "duration" or p2 != "instant":
                 return {"valid": False, "confidence": "LOW", "reason": f"Period type mismatch for return: {p1}/{p2}"}
             if "end" not in den.metric_key and "assets" not in den.metric_key and "equity" not in den.metric_key:
    ...
    def _serialize_highlight(self, obj):
        return {
            "value": float(obj.value_numeric) if obj.value_numeric is not None else None,
            "unit": obj.unit_label,
    ...
    def _upsert_derived(self, **kwargs):
        ...
        try:
            ...
            self.db.rollback()
            logger.error(f"Upsert Error {kwargs['doc_id']}/{kwargs['derived_key']}: {e}")
