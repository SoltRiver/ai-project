            ...
            try:
                ...
                if response.status_code in [429, 500, 502, 503, 504]:
                    if attempt < retries:
                        ...
                        logger.warning(f"EDINET API Error {response.status_code}. Retrying in {sleep_time:.2f}s... (Attempt {attempt + 1}/{retries})")
                        await asyncio.sleep(sleep_time)
                        continue
                
                # If we reach here, it's a non-retryable error or retries exhausted
                logger.error(f"EDINET API Failed (Non-retryable). Status: {response.status_code}, Body: {response.text[:200]}")
