...
class SectorMapper:
    ...
    def _load_config(self):
        ...
        if not os.path.exists(config_path):
            logger.warning(f"Sector config not found at {config_path}. Using empty mapping.")
            self._config = {}
            return
        ...
        try:
            ...
            logger.info(f"Loaded {len(self._mapping_dict)} sector mappings (Version: {self._config.get('version')})")
    ...
    def get_internal_sector_name(self, edinet_name: str) -> str:
        ...
        # Fallback
        logger.warning(f"Unknown EDINET sector '{edinet_name}'. Mapping to default '{self._default_sector}'.")
        return self._default_sector
