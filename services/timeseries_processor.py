...
class TimeseriesProcessor:
    ...
    def _select_annual_docs(self, sec_code: str) -> Dict[int, EdinetDocument]:
        ...
        docs = self.db.query(EdinetDocument).filter(
            EdinetDocument.sec_code == sec_code,
            EdinetDocument.period_end != None
        ).order_by(EdinetDocument.period_end).all()
        ...
        for year, year_docs in candidates_by_year.items():
            ...
            filtered_docs.sort(key=lambda x: x.submit_datetime or x.created_at, reverse=True)
    ...
    def _populate_timeseries(self, sec_code: str, year_doc_map: Dict[int, EdinetDocument]):
        for year, doc in year_doc_map.items():
            # Fetch highlights
            highlights = self.db.query(EdinetFinancialHighlight).filter_by(doc_id=doc.doc_id).all()
    ...
    def _calculate_comparisons(self, sec_code: str):
        ...
        series = self.db.query(EdinetMetricTimeseries).filter_by(sec_code=sec_code)\
            .order_by(EdinetMetricTimeseries.metric_key, EdinetMetricTimeseries.period_end_year).all()
        
        # Group by metric
        grouped = {}
        for s in series:
            if s.metric_key not in grouped: grouped[s.metric_key] = []
        ...
        for key, rows in grouped.items():
            ...
            for r in rows:
                history_vals[r.period_end_year] = float(r.value_numeric) if r.value_numeric is not None else None
                history_rows[r.period_end_year] = r
                
            for i, curr in enumerate(rows):
                year = curr.period_end_year
                val = float(curr.value_numeric) if curr.value_numeric is not None else None
    ...
    def _upsert_timeseries(self, **kwargs):
        try:
            existing = self.db.query(EdinetMetricTimeseries).filter_by(
    ...
    def _upsert_comparison(self, **kwargs):
        try:
            existing = self.db.query(EdinetMetricComparison).filter_by(
                sec_code=kwargs['sec_code'],
                metric_key=kwargs['metric_key'],
                period_end_year=kwargs['period_end_year']
            ).first()
